...
async function getImages(req: Request, res: Response) {
	...
	const query = url.parse(req?.query?.imageName as string, true).query;
	const file = url.parse(req?.query?.imageName as string).pathname;

	if (
		!fs.existsSync(path.resolve(__dirname, "../../../../public/images/" + file))
	) {
